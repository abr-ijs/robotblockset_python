"""Robot interfaces for interactive Python MuJoCo scenes.

This module provides RobotBlockSet robot backends built on `scene_pymujoco`,
including the generic PyMuJoCo robot interface, concrete robot wrappers, and
an internal joint-control variant for selected robots.
...
class robot_pymujoco(robot):
    """
    PyMuJoCo-backed robot interface operating on a :class:`~robotblockset.mujoco.scene_pymujoco.mujoco_scene`.

    Attributes
    ----------
    scene : :class:`~robotblockset.mujoco.scene_pymujoco.mujoco_scene`
    ...
    def __init__(self, robot_name: str, scene: mujoco_scene = None, **kwargs: Any) -> None:
        ...
        **kwargs : Any
        ...
        -------
        None
            This constructor initializes the PyMuJoCo robot interface in place.
        """
        ...
        if isinstance(kwargs["ActuatorNames"], str):
            if kwargs["ActuatorNames"].lower() == "auto":
                ...
                raise ValueError(f"Argument 'ActuatorNames':{kwargs['ActuatorNames']} is invalid. Only `auto` or `ori`¸ are accepted.")
    ...
    def SetStrategy(self, strategy: str) -> None:
        ...
        Notes
        -----
        The PyMuJoCo wrapper currently keeps a single joint-position strategy
        and accepts this method for compatibility.
        """
        pass
...
class panda(robot_pymujoco, panda_spec):
    """PyMuJoCo robot wrapper for the Franka Panda manipulator."""
...
class fr3(robot_pymujoco, fr3_spec):
    """PyMuJoCo robot wrapper for the Franka Research 3 manipulator."""
...
class lwr(robot_pymujoco, lwr_spec):
    """PyMuJoCo robot wrapper for the KUKA LWR manipulator."""
...
class iiwa(robot_pymujoco, iiwa_spec):
    """PyMuJoCo robot wrapper for the KUKA iiwa manipulator."""
...
class ur10(robot_pymujoco, ur10_spec):
    """PyMuJoCo robot wrapper for the Universal Robots UR10 manipulator."""
...
class ur10e(robot_pymujoco, ur10e_spec):
    """PyMuJoCo robot wrapper for the Universal Robots UR10e manipulator."""
...
class ur5(robot_pymujoco, ur5_spec):
    """PyMuJoCo robot wrapper for the Universal Robots UR5 manipulator."""
...
class ur5e(robot_pymujoco, ur5e_spec):
    """PyMuJoCo robot wrapper for the Universal Robots UR5e manipulator."""
...
class crx20(robot_pymujoco, crx20_spec):
    """PyMuJoCo robot wrapper for the FANUC CRX-20 collaborative manipulator."""
...
class hc20(robot_pymujoco, hc20_spec):
    """PyMuJoCo robot wrapper for the Yaskawa HC20 manipulator."""
...
class z1(robot_pymujoco, z1_spec):
    """PyMuJoCo robot wrapper for the Unitree Z1 arm."""
...
class b2(robot_pymujoco, b2_spec):
    """PyMuJoCo robot wrapper for the Unitree B2 platform-arm system."""
...
class robot_pymujoco_joint_control(robot_pymujoco):
    """
    PyMuJoCo robot interface with internal joint-space impedance control.
...
class panda_joint_control(robot_pymujoco_joint_control, panda_spec):
    """
    PyMuJoCo Panda robot with internal joint-space impedance control.
